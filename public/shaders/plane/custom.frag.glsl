...
void main() {
    ...
    // Layer 1: Slow horizontal drift
    float n1 = fbm(uv + vec2(uTime * 0.1, uTime * 0.05));

    // Layer 2: Vertical rise
    float n2 = fbm(uv + vec2(uTime * 0.05, -uTime * 0.15));

    // Combine layers with offset
    float noise = fbm(uv + vec2(n1 * 0.5, n2 * 0.8));

    // Add some distortion for blob-like movement
